...
export function RunTests() {
  ...
  let abil = forest.abilities[0] as ActivatedAbility;
  abil.activate(forest);
  StackManager.resolveNext();
  p1.play(p1.zones.hand[0]);
  StackManager.resolveNext();
  let elves = Battlefield[1] as Creature;
  ...
  let adapCard = adap.representedCard as ForcedAdaptationCard;
  ...
  let abil2 = forest2.abilities[0] as ActivatedAbility;
  abil2.activate(forest2);
  StackManager.resolveNext();
  p2.play(p2.zones.hand[0]);
  StackManager.resolveNext();
  let elves2 = Battlefield[4] as Creature;
