...
class ManaSymbol {
  color = '';
  canPayFor = (c: Card, a: boolean) => true;
  limited = false;
  floating = false;
  keep: boolean;
  constructor(col: string, cpf?: (payFor: Card, isAbility: boolean) => boolean, keep = false) {
    if (cpf) this.canPayFor = cpf;
    this.color = col;
    this.keep = keep;
  }
  get priority() {
...
export class ManaCost extends Cost {
  ...
  card: Card;
  isAbility: boolean;
  constructor(mana: Mana | object, card?: Card, isAbility: boolean = false) {
    super();
...
export class Mana {
  ...
  constructor(
    colors: object = {},
    cpf?: (payFor: Card, isAbility: boolean) => boolean,
    keep = false
  ) {
  ...
  get value() {
    let v = 0;
    for (let i of Object.values(this.colors)) v += i;
    return v;
  }
  get asString() {
    let s: string = this.colors["generic"] ? this.colors["generic"].toString() : "";
    for (let i of Object.keys(this.colors)) s += (i == "blue" ? "U" : i[0].toUpperCase()).repeat(this.colors[i]);
    if (!s.length) s = "nothing";
    return s;
  }
  get asHTML() {
    let order = ["white", "blue", "black", "red", "green"];
    let s: number = this.colors["generic"] || 0;
  ...
  pay(thing: Card | ManaCost | object, spend = true) {
    ...
    for (let c = 0; c < cost['generic'] || 0; c++) {
      if (!sym.length) throw new Error('Problem while paying generic mana!');
      let best = sym.sort((a, b) =>
        a.priority > b.priority ? -1 : b.priority > a.priority ? 1 : 0
      )[0];
      sym.splice(sym.indexOf(best), 1);
    }
    // Woot!
    if (spend) this.symbols = sym;
    console.log((spend ? "Paid " : "Can pay ") + JSON.stringify(cost) + (card ? " (" + card.name + ")" : "") + " successfully. New symbols: " + JSON.stringify(this.symbols));
    return true;
  }
}

export function ManaFromSymbols(symbols: ManaSymbol[]) {
  let col = {};
