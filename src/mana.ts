...
export interface ManaPoolPart {
  ...
  green?: number;
  colorless?: number;
  canPayFor?: (payFor: Card, isAbility: boolean) => boolean;
  keep?: boolean;
}

export interface ManaObject {
  required?: SimpleManaObject;
  choices?: SimpleManaObject[][];
}
...
export class ManaPool {
  mana: ManaPoolPart[];
  constructor(mana: ManaPoolPart | ManaPoolPart[] = {}) {
  ...
  pay(thing: Card | ManaObject, player: Player, spend = true) {
    ...
    if (pay.success) this.mana = pay.remain.mana;
    ...
    while (cost.choices.length && keepGoing) {
      ...
      if (worked && this.value >= (generic + cost.required.generic || 0)) validPaths.push(structuredClone(combo)); // deep copy again
      combo[0]++;
      while (combo.filter((x, i) => x >= cost.choices[i].length).length) {
        ...
        if (problem + 1 == combo.length) { keepGoing = false; break; }
    ...
    for (let i of obvious) {
      ...
      if (pay.success) this.mana = pay.remain.mana;
      else throw new Error("I think there is a problem");
    }
  ...
  add(other: ManaPoolPart | ManaPoolPart[] | ManaPool) {
    if (other instanceof ManaPool) this.mana = [...this.mana, ...other.mana];
    else if (Array.isArray(other)) this.mana = [...this.mana, ...other];
    else this.mana.push(other);
  }
  get asString() {
...
function simplePay(mana: ManaPool, cost: SimpleManaObject, tryPayingGeneric = false) {
  ...
  for (let col of keys) {
    ...
    // Is there enough?
    if (payables.reduce((a, b) => a + b[col], 0) < toPay) return { success: false, remain: new ManaPool(mana.mana) };
    // Now pay
    for (let i of payables) {
      if (i[col] >= toPay) { i[col] -= toPay; break; }
      else { toPay -= i[col]; i[col] = 0; }
    }
  }
