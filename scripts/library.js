...
import { SimpleActivatedAbility, FirstStrikeAbility, VigilanceAbility, TrampleAbility, DoubleStrikeAbility, HasteAbility } from "./ability.js";
...
import { PlayCardHook, BeginStepHook, StatsHook, ProtectionAbility, FlyingAbility, HeroicAbility, IndestructibleAbility, TypesHook, AbilitiesHook, MenaceAbility, TakeDamageHook, CardClickHook, SelectTargetsHook, WardAbility, FinishedResolvingSpellHook, LifelinkAbility } from "./hook.js";
 ...
 */
let typePredicate = function (validate) {
    return function (x) {
        return validate(x);
    };
};
...
export class BasicLandCard extends PermanentCard {
    ...
    constructor(name, color) {
        super(name, ["Basic", "Land", name], "{A1}{T}{EC1}Add {" + (color == Color.blue ? "U" : color[0].toUpperCase()) + "}.{EA1}", undefined, new SimpleActivatedAbility(new TapCost(), card => new AddManaEffect({ [color]: 1 }).resolve(card)));
        this.color = color;
    }
}
...
export class GiantGrowthCard extends SimpleSpellCard {
    constructor() {
        super("Giant Growth", ["Instant"], "Target creature gets +3/+3 until end of turn.", typePredicate(x => x instanceof Creature), (self, target) => {
...
export class FigureOfDestinyCard extends CreatureCard {
    constructor() {
        super("Figure of Destiny", ["Creature", "Kithkin"], `{A1}({R}/{W}){EC1}{CARDNAME} becomes a 2/2 Kithkin Spirit.{EA1}<br/>
        {A2}3 x ({R}/{W}){EC2}If {CARDNAME} is a Spirit, it becomes a 4/4 Kithkin Spirit Warrior.{EA2}<br/>
        {A3}6 x ({R}/{W}){EC3}If {CARDNAME} is a Warrior, it becomes a 8/8 Kithkin Spirit Warrior Avatar with flying and first strike.{EA3}`, 1, 1, new ManaCost({ choices: [[{ red: 1 }, { white: 1 }]] }), [
...
export class GeneralFerrousRokiricCard extends CreatureCard {
    constructor() {
        super("General Ferrous Rokiric", ["Creature", "Legendary", "Human", "Soldier"], "Protection from monocolored. Whenever you cast a multicolored spell, create a 4/4 red and white Golem token.", 3, 1, new ManaCost({ red: 1, white: 1, generic: 1 }), [
...
export class AnaxAndCymedeCard extends CreatureCard {
    constructor() {
        super("Anax and Cymede", ["Creature", "Legendary", "Human", "Soldier"], `First strike, vigilance.<br/>
      Whenever you cast a spell that targets {CARDNAME},<br/>
      creatures you control get +1/+1 and gain trample until end of turn.`, 3, 2, new ManaCost({ red: 1, white: 1, generic: 1 }), [
...
export class FeatherTheRedeemedCard extends CreatureCard {
    constructor() {
      ...
      If you do, return it to your hand at the beginning of the next end step.`, 3, 4, new ManaCost({ red: 1, white: 2 }), new FinishedResolvingSpellHook((me, orig, card, player, targets) => {
...
export class IroasGodOfVictoryCard extends CreatureCard {
    constructor() {
        super("Iroas, God of Victory", ["Creature", "Enchantment", "Legendary", "God"], `Indestructible. As long as your devotion to red and white is less than seven, {CARDNAME} isn't a creature.<br/>
...
export class RadiantScrollwielderCard extends CreatureCard {
    constructor() {
        super("Radiant Scrollwielder", ["Creature", "Dwarf", "Cleric"], `Instant and sorcery spells you control have lifelink.<br/>
                ...
                if (that.step == Step.upkeep && that.currentPlayer.is(me.controller)) {
                    ...
                    new ApplyAbilityEffect(new CardClickHook((me2, orig2, that2) => {
                        ...
                        new ApplyAbilityEffect(new FinishedResolvingSpellHook((me3, orig3, that3, player, targets) => {
                            if (!that3.is(spell))
                                return orig3(that3, player, targets);
                            return Zone.exile;
                        })).resolve(me2);
                        that2.play();
                    })).queue(me);
...
export class ZadaHedronGrinderCard extends CreatureCard {
    constructor() {
        super("Zada, Hedron Grinder", ["Creature", "Legendary", "Goblin", "Ally"], `Whenever you cast an instant or sorcery spell that targets only {CARDNAME},<br/>
...
export class FloweringOfTheWhiteTreeCard extends PermanentCard {
    constructor() {
        super("Flowering of the White Tree", ["Legendary", "Enchantment"], `Legendary creatures you control get +2/+1 and have ward {1}.<br/>
...
export class LightningBoltCard extends SimpleSpellCard {
    constructor() {
        super("Lightning Bolt", ["Instant"], "{CARDNAME} deals 3 damage to any target.", typePredicate(x => x instanceof Creature || x instanceof Player || x instanceof Planeswalker), (self, target) => { target.takeDamage(self, 3); }, new ManaCost({ red: 1 }));
    }
    makeEquivalentCopy = () => new LightningBoltCard();
}
export class RecklessRageCard extends SpellCard {
    constructor() {
        super("Reckless Rage", ["Instant"], "{CARDNAME} deals 4 damage to target creature you don't control and 2 damage to target creature you control.", (self, targets) => targets.length == 2 && targets[0] instanceof Creature && !targets[0].controller.is(self.controller) && targets[1] instanceof Creature && targets[1].controller.is(self.controller), (self, field) => field.filter(x => x instanceof Creature && !x.controller.is(self.owner)).length > 0 && field.filter(x => x instanceof Creature && x.controller.is(self.owner)).length > 0, (self, targets) => {
...
export class BorosCharmCard extends SpellCard {
    constructor() {
        super("Boros Charm", ["Instant"], `Choose one:<br/>
...
export class LightningHelixCard extends SimpleSpellCard {
    constructor() {
        super("Lightning Helix", ["Instant"], "{CARDNAME} deals 3 damage to any target and you gain 3 life.", typePredicate(x => x instanceof Creature || x instanceof Player || x instanceof Planeswalker), (self, target) => { target.takeDamage(self, 3); self.controller.gainLife(self, 3); }, new ManaCost({ red: 1, white: 1 }));
    }
...
class IntegrityCard extends SimpleSpellCard {
    constructor() {
        super("Integrity", ["Instant"], "Target creature gets +2/+2 until end of turn.", typePredicate(x => x instanceof Creature), (self, target) => {
            new ApplyAbilityEffect(new StatsHook((me, orig, that, stat) => {
                if (!me.is(that))
                    return orig(that, stat);
...
class InterventionCard extends SimpleSpellCard {
    constructor() {
        super("Intervention", ["Instant"], "{CARDNAME} deals 3 damage to any target and you gain 3 life.", typePredicate(x => x instanceof Creature || x instanceof Player || x instanceof Planeswalker), (self, target) => { target.takeDamage(self, 3); self.controller.gainLife(self, 3); }, new ManaCost({ red: 1, white: 1, generic: 2 }));
    }
...
export class RipApartCard extends SpellCard {
    constructor() {
        super("Rip Apart", ["Sorcery"], `Choose one:<br/>
      * 3 damage to target creature or planeswalker.<br/>
      * Destroy target artifact or enchantment`, (self, targets) => targets.length == 1 && targets[0] instanceof Creature || targets[0] instanceof Planeswalker || (targets[0] instanceof Permanent && (targets[0].hasType("Artifact") || targets[0].hasType("Enchantment"))), (self, field) => field.filter(x => x instanceof Creature || x instanceof Planeswalker || x.hasType("Artifact") || x.hasType("Enchantment")).length > 0, (self, targets) => {
            let t = targets[0];
            if (t instanceof Permanent && (t.hasType("Artifact") || t.hasType("Enchantment")))
                t.destroy();
            else if (t instanceof Creature || t instanceof Planeswalker)
                t.takeDamage(self, 3);
        }, new ManaCost({ red: 1, white: 1 }));
    }
    makeEquivalentCopy = () => new BorosCharmCard();
...
export class ThrillingDiscoveryCard extends UntargetedSpellCard {
    constructor() {
        super("Thrilling Discovery", ["Sorcery"], `You gain 2 life. Then you may discard 2 cards to draw 3 cards.`, (self) => {
...
export class AngelfireIgnitionCard extends SimpleSpellCard {
    constructor() {
        super("Angelfire Ignition", ["Sorcery"], `Put two +1/+1 counters on target creature.<br/>
      It gains vigilance, trample, lifelink, haste, and indestructible this turn.`, typePredicate(x => x instanceof Creature), (self, target) => {
